...
class GetApprovedReviews implements IReadDb
{
    /**
    ...
    {
        $sth = $this->dbh->prepare(/** @lang MariaDB */"SELECT * FROM reviews where object_id = :object_id && object_type = :object_type && status = :status order by publish_datetime_utc ");
        $sth->execute(array(
