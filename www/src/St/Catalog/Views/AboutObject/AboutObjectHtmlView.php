...
class AboutObjectHtmlView extends HtmlView implements IView
{
    ...
    #[\Override] public function out(): void
    {
                    ...
                    <div class="d-none">
                        <?php foreach (array_slice($this->getCatalogObject()->getAdditionalImages(999), 4) as $image):?>
                            <a href="<?php print !str_starts_with($image->getUri(2000, 2000), "/") ? "/" : ""; ?><?php print $image->getUri(2000, 2000);?>" data-fancybox="sight-<?php print $this->catalog_object->getObjectId();?>">
                                <img alt="" class="w-100 h-100 object-fit-cover" src="<?php print !str_starts_with($image->getUri(296, 296), "/") ? "/" : ""; ?><?php print $image->getUri(296, 296, true)?>">
                            </a>
                        <?php endforeach;?>
                    </div>
                <?php endif;?>
        ...
        <div class="section-object__description reviews mt-3">
            <h3>Отзывы</h3>
            <?php if (!sizeof($this->catalog_object->getApprovedReviews())): ?>
        ...
        <?php if (Auth::getInstance()->get()):?>
            <?php (new AddReviewModalDialogHtmlView( Auth::getInstance()->get(), $this->catalog_object->getObjectId(), ReviewObjectTypesEnum::Object ))->out();?>
        <?php endif; ?>
